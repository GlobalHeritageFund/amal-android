...
class Promise<T> private constructor(state: State<T>) {
    ...
    constructor(error: Error) : this(State.Rejected(error))
    ...
    fun catch(onError: (Error) -> Unit): Promise<T> {
        this.addCallback(
                { value ->
    ...
    fun <V> flatMap(flatMap: (T) -> Promise<V>): Promise<V> {
        return Promise<V>({ fulfill, reject ->
            this.addCallback(
                    { value ->
                        try {
