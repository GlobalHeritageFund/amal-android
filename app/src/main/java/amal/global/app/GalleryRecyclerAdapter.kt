...
class GalleryRecyclerViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
    ...
    private val dividerTextView: TextView? = itemView.findViewById(R.id.divider_text)
    // For selection in Adapter's onBindViewHolder
    val selectionStateImageView: ImageView? = itemView.findViewById(R.id.selectionStateImageView)

    private fun bindPhoto(item: GalleryItem.GalleryPhoto) {
        item.photoToShow.load(itemView.context).centerCrop().into(contentImageView ?: return)
    }

    private fun bindDivider(item: GalleryItem.GalleryDateDivider) {
        dividerTextView?.text = item.photoGroupDate
    }
...
class GalleryRecyclerAdapter(private val context: Context): RecyclerView.Adapter<GalleryRecyclerViewHolder>() {
    ...
    override fun onBindViewHolder(holder: GalleryRecyclerViewHolder, position: Int) {
        ...
        val cellView = holder.selectionStateImageView // New (assuming selectionStateImageView is part of TYPE_PHOTO layout)
        if (cellView != null && holder.itemViewType == TYPE_PHOTO) { // Check if cellView is part of this item type
            val photoItem = galleryItems[position] as GalleryItem.GalleryPhoto // Safe due to itemViewType check
            val isSelected = selectedImagePaths.contains(photoItem.photoToShow.filePath)
            if (isSelected) cellView.setImageResource(R.drawable.ic_img_select_active)
    ...
    fun selectedItems(): List<LocalImage> {
        return galleryItems.mapNotNull { galleryItem ->
            if (galleryItem is GalleryItem.GalleryPhoto && selectedImagePaths.contains(galleryItem.photoToShow.filePath)) {
                galleryItem.photoToShow
            } else {
                null
            }
        }
    ...
    fun toggleSelectionAt(position: Int) {
        ...
        if (item is GalleryItem.GalleryPhoto) {
            val filePath = item.photoToShow.filePath // Ensure filePath exists, but since non-nullable, it should be safe to use
            if (selectedImagePaths.contains(filePath)) {
