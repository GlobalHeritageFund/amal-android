...
//may not need all of this bc of redefining enum but no neet do change for now
class RestTargetAdapter {
    @ToJson
    fun toJson(restTarget: RestTarget?): String? {
        Log.d("ReportDraft", "Called Rest ToJason")
        return restTarget?.phrase
    }

    @FromJson
    fun fromJson(restTargetPhrase: String?): RestTarget? {
        Log.d("ReportDraft","Called Rest fromJason")
        if (restTargetPhrase.isNullOrEmpty()) return null
        return RestTarget.fromPhrase(restTargetPhrase)
    }
}
